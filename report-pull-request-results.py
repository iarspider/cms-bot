...
def parse_workflow_info(parts, relval_dir):
    ...
    workflow_info["message"] = get_wf_error_msg(
        join(relval_dir, out_directory, out_file)
    )
...
def cmd_to_addon_test(command, addon_dir):
    try:
        ...
        e, o = run_cmd(
            "ls -d %s/%s/%s 2>/dev/null | tail -1" % (addon_dir, addon_subdir, logfile)
        )
    except:
...
def read_addon_log_file(unit_tests_file):
    ...
    for line in openlog(unit_tests_file):
        ...
        if ": FAILED -" in line:
            cnt += 1
            tname, err = cmd_to_addon_test(
                line.split(": FAILED -")[0].strip(), addon_dir
            )
            if not tname:
                tname = "unknown"
            else:
...
def send_comparison_ready_message(
    ...
    wfs_das_inconsistency = (
        openlog(wfs_with_das_inconsistency_file)
        .readline()
        .rstrip()
        .rstrip(",")
        .split(",")
    ...
    alt_comp_dir = join(
        dirname(comparison_errors_file), "upload", "alternative-comparisons"
    )
    print("Alt comparison directory: ", alt_comp_dir)
    if exists(alt_comp_dir):
        err, out = run_cmd(
            "grep ' Compilation failed' %s/runDQMComp-*.log" % alt_comp_dir
        )
        print(out)
        if not err:
            err_wfs = {}
