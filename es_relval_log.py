...
def es_parse_jobreport(payload, logFile):
    ...
    if not os.path.exists(xmlFile):
        ...
        return payload
    payload["jobreport"] = (
        "/".join(payload["url"].split("/")[:-1]) + "/" + xmlFile.split("/")[-1]
    )
    ...
    for i in reports_p:
        ...
        for j in summaries:
            if (
                j.get("Metric") == "SystemMemory"
                or j.get("Metric") == "StorageStatistics"
            ):
                continue
...
def es_parse_log(logFile):
    ...
    if datasets:
        ...
        for ds in datasets:
            ...
            send_payload(
                "ib-dataset-" + week, "relvals-dataset", idx, json.dumps(dataset)
            )
