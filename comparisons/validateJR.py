...
def autoLoadEnabler():
    if os.path.isfile(
        os.path.join(
            ...
            "src/FWCore/FWLite/interface/FWLiteEnabler.h",
        )
    ):
...
def file_processes(fileName):
    ...
    raw_proc = (
        os.popen(
            "grep -e 'Processing History:' -A %s %s | awk '{print $1}'"
            % (max_proc, prov_file)
        )
        .read()
        .split("\n")[1:]
...
if __name__ == "__main__":
    ...
    parser.add_option(
        ...
        dest="base",
        default="base/",
        help="path to the file to compare with ref",
    )
    parser.add_option(
        "--ref", dest="ref", default="ref/", help="path to the reference files"
    )
    parser.add_option(
        "--wf",
        dest="workflow",
    ...
    all_output_root_files = glob.glob(
        "%s/%s_*/step*.root" % (options.ref, options.workflow)
    )
    ...
    if options.procs == 0:
        ...
        results = [
            threads.apply_async(process_file, (f,)) for f in all_output_root_files
        ]
        for r in results:
            r.wait()
