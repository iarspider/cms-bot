...
def es_send_external_stats(
    ...
    file_stamp = int(
        tstat(stats_dict_file_path).st_mtime
    )  # get the file stamp from the file
    week = str((file_stamp / 86400 + 4) / 7)
    with open(opts_dict_file_path, "r") as opts_dict_f:
        opts_dict = json.load(opts_dict_f)
    index_sha = sha1(
        "".join([str(x) for x in opts_dict.values()]) + stats_dict_file_path
    ).hexdigest()
    ...
    try:
        send_payload(
            es_index_name + "-" + week, es_doc_name, index_sha, json.dumps(sdata)
        )
    except Exception as e:
        print(e.message)
...
def getExternalsESstats(
    arch="*", externalsList="", lastNdays=30, page_size=0, fields=None
):
    externals_names = ""
    if externalsList == "":
        externals_names = "*"
...
def orderStatsByName(externalsStats=None):
    ...
    for ext in namedStats:
        namedStats[ext] = sorted(
            namedStats[ext], key=lambda x: x["@timestamp"], reverse=True
        )
    return namedStats


def get_externals_build_stats(
    arch="*", lastNdays=30, cpus=0, memoryGB=0, default_key="90"
):
    default_keys = {
        "cpu": "cpu_" + default_key,
    ...
    for name in all_data:
        ...
        for k in default_keys:
            if all_data[name][k]:
                data["packages"][name][k] = int(
                    sum(all_data[name][k]) / len(all_data[name][k])
                )
