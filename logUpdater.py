...
class LogUpdater(object):
    def __init__(
        self, dirIn=None, dryRun=False, remote=None, webDir="/data/sdt/buildlogs/"
    ):
        if not remote:
    ...
    def getDoneRelvals(self):
        ...
        destination = os.path.join(
            self.webTargetDir, "pyRelValPartialLogs", "*", wfDoneFile
        )
        code, out = self.runRemoteCmd("ls " + destination, debug=False)
        return [
            wf.split("/")[-2].split("_")[0]
            for wf in out.split("\n")
    ...
    def updateAddOnTestsLogs(self):
        ...
        self.copyLogs(
            "addOnTests.pkl",
            "addOnTests/logs",
            os.path.join(self.webTargetDir, "addOnTests/logs"),
        )
        return
    ...
    def updateProductionRelValLogs(self, workFlows):
        print(
            "\n--> going to copy Production RelVals logs to",
            self.webTargetDir,
            "... \n",
        )
    ...
    def copyLogs(self, what, logSubDir="", tgtDirIn=None):
        ...
        self.copy2Remote(
            os.path.join(self.cmsswBuildDir, logSubDir, what), tgtDirIn + "/"
        )
