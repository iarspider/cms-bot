...
def process_relvals(
    threads=None, cmssw_version=None, arch=None, cmssw_base=None, logger=None
):
    pass
...
if __name__ == "__main__":
    ...
    parser.add_option(
        "-i", "--id", dest="jobid", help="Job Id e.g. 1of3", default="1of1"
    )
    parser.add_option(
        "-l",
        "--list",
    ...
    if re.match("^CMSSW_(9_([3-9]|[1-9][0-9]+)|[1-9][0-9]+)_.*$", cmssw_ver):
        ...
        if opts.workflow:
            stime = time()
            p = Popen(
                "%s/jobs/create-relval-jobs.py %s" % (SCRIPT_DIR, opts.workflow),
                shell=True,
            )
            e = waitpid(p.pid, 0)[1]
