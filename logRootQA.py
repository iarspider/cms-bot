...
if run in ["all", "events"]:
    ...
    if lines > 0:
        ...
        print(
            "SUMMARY You potentially removed "
            + str(-1 * lines)
            + " lines from the logs"
        )
    else:
    ...
    if not sameEvts:
        qaIssues = True
        print(
            "SUMMARY ROOTFileChecks: Some differences in event products or their sizes found"
        )
    print("\n")
    if run == "events":
        sys.exit(0)
...
if run in ["all", "JR"]:
    ...
    print(
        "SUMMARY Reco comparison results:",
        nDiff,
        "differences found in the comparisons",
    )
    if nAll != nOK:
...
print(
    "SUMMARY DQMHistoSizes: Histogram memory added:",
    newDQM,
    "KiB(",
    nDQM,
    "files compared)",
