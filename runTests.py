...
class UnitTester(IBThreadBase):
    ...
    def checkUnitTestLog(self):
        ...
        try:
            ...
            pass
        tlc = checkTestLog.TestLogChecker(
            self.startDir + "/unitTests-summary.log", True
        )
    ...
    def splitUnitTestLogs(self):
        ...
        tls = splitUnitTestLog.LogSplitter(
            self.startDir + "/unitTests-summary.log", True
        )
            ...
            for packDir in glob.glob(testLog + "*/*"):
                ...
                if os.path.exists(packDir):
                    err, testFiles = run_cmd(
                        "find "
                        + packDir
                        + " -maxdepth 2 -mindepth 2 -name testing.log -type f"
                    )
...
class LibDepsTester(IBThreadBase):
    ...
    def run(self):
        ...
        try:
            ...
            if ret != 0:
                print(
                    "ERROR when running lib dependency check: cmd returned " + str(ret)
                )
        except Exception as e:
...
class ReleaseProductsDump(IBThreadBase):
    ...
    def run(self):
        ...
        cmd = (
            "cd "
            + self.startDir
            + "; RelProducts.pl > ReleaseProducts.list  2> "
            + rperrFileName
        )
        ret = runCmd(cmd)
        if ret != 0:
...
class BuildFileDependencyCheck(IBThreadBase):
    ...
    def run(self):
        ...
        if ret != 0:
            print(
                "ERROR when running BuildFileDependencyCheck: cmd returned " + str(ret)
            )
        ...
        self.logger.updateLogFile(
            self.startDir + "/depViolationSummary.pkl", "testLogs"
        )
...
class CodeRulesChecker(IBThreadBase):
    ...
    def run(self):
        ...
        cmd = (
            "cd "
            + self.startDir
            + "; rm -rf  codeRules; mkdir codeRules; cd codeRules; "
        )
        cmd += (
    ...
    def runDuplicateDictCheck(self, deps=None):
        ...
        for opt in ["dup", "lostDefs", "edmPD"]:
            ...
            try:
                ...
                print(
                    "ERROR during test duplicateDictCheck : caught exception: " + str(e)
                )
