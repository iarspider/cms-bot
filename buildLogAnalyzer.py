...
class LogFileAnalyzer(object):
    ...
    def makeHTMLSummaryPage(self):
        ...
        htmlFile.write(
            "<h3> found "
            + str(totErr)
            + " errors and warnings in total, by type: </h3>\n"
        )
            ...
            for pkg in pkgList:
                ...
                for pKey in keyList:
                    ...
                    if pKey in pkg.errSummary.keys():
                        if sys.version_info[0] < 3:
                            htmlFile.write(
                                str(pkg.errSummary[pKey]).decode("ascii", "ignore")
                            )
                        else:
    ...
    def analyzeFile(self, fileNameIn):
        ...
        errorInf = [
            ...
            {
                str(
                    "^gmake: \*\*\* .*?/src/" + subsys + "/" + pkg + "/bin/(.*?)/.*?\.o"
                ): ["compError", "for executable %s"]
            },
            {
            ...
            },
            {
                str(
                    ...
                    + pkg
                    + "/test/stubs/.*?\.o"
                ): ["compError", "for library in test/stubs"]
            },
            {
                str(
                    "^gmake: \*\*\* .*?/src/"
                    ...
                    + pkg
                    + "/test/(.*?)/.*?\.o"
                ): ["compError", "for executable %s in test"]
            },
            {
                str(
                    "^gmake: \*\*\* .*?/src/"
            ...
            {
                str(
                    "^gmake: \*\*\* .*?/src/" + subsys + "/" + pkg + "/test/(.*?)\.o"
                ): ["compError", "for executable %s in test"]
            },
            {
                str(
                    "^gmake: \*\*\* .*?/src/" + subsys + "/" + pkg + "/test/(.*?)/\1"
                ): ["linkError", "for executable %s in test"]
            },
            {
                str(
                    "^gmake: \*\*\* .*?/src/"
                    ...
                    + pkg
                    + "/plugins/(.*?)/.*?\.o"
                ): ["compError", "for plugin %s in plugins"]
            },
            {
                str(
                    "^gmake: \*\*\* .*?/src/"
            ...
            {str("^ValueError: .*"): ["pythonError", "value error in module"]},
            {
                ...
                ): ["dwnlError", "for file in data/download.url in test"]
            },
            {
                str(
                    "^ */.*?/"
