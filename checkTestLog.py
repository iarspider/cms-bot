...
class TestLogChecker(object):
    ...
    def checkScramWarnings(self, logFile, verbose=False):
        ...
        exprNoPkg = (
            "^WARNING: Unable to find package/tool called ([A-Za-z].*/[A-Za-z].*)"
        )
        ...
        noExportRe = re.compile(
            "^WARNING: ([A-Za-z].*)/BuildFile does not export anything:"
        )
        ...
        self.outFile.write(
            "found " + str(len(noToolPkgs)) + " BuildFiles with tool problems\n"
        )
        if verbose:
            for p in noToolPkgs:
    ...
    def check(self, logFile):
        ...
        self.outFile.write(
            "total number of tests: " + str(len(list(results.keys()))) + "\n"
        )
        nMax = 1000
