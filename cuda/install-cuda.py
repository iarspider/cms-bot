...
class Component:
    ...
    def fill(self, catalog, key, os_arch):
        ...
        if key not in catalog:
            raise RuntimeError(
                f"the component '{key}' is not available in the JSON catalog"
            )
...
def fix_symlinks(install_dir, dry_run=False):
    ...
    for xdir in libdirs:
        for dirname in glob.glob(xdir):
            for lib in [
                os.path.basename(l)
                for l in glob.glob(os.path.join(dirname, "lib*.so*"))
            ]:
...
def movetree(src, dst, overwrite=False):
    ...
    # Other combinations are not supported: directories cannot be merged with files or links, etc.
        ...
        raise RuntimeError(
            f"Cannot merge or overwrite entries of different types: {src} vs {dst}"
        )
...
def download_package(base_url, download_dir, component):
    ...
    if os.path.isfile(target):
        try:
            ...
            info(
                f"file {target} exists and matches the expected size, checksum and hash."
            )
            return target
...
def unpack_package(package, local_dir):
    ...
    for info in archive:
        if not os.path.normpath(os.path.join(local_dir, info.name)).startswith(
            archive_dir
        ):
            raise RuntimeError(
...
def main():
    ...
    for arg in args.version:
        ...
        if "https://" in arg:
            version = (
                os.path.basename(arg).replace("redistrib_", "").replace(".json", "")
            )
            url = arg
