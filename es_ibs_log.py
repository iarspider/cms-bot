...
def process_addon_log(logFile):
    ...
    payload["@timestamp"] = timestp
    payload["name"] = (
        pathInfo[-1].split("-")[1].split("_cmsRun_")[0].split("_cmsDriver.py_")[0]
    )
...
# get log files
logs = run_cmd(
    "find /data/sdt/buildlogs -mindepth 6 -maxdepth 6 -name 'unitTests-summary.log'"
)
logs = logs[1].split("\n")
# process log files
for logFile in logs:
...
logs = run_cmd(
    "find /data/sdt/buildlogs -mindepth 6 -maxdepth 6 -name 'unitTestLogs.zip'"
)
logs = logs[1].split("\n")
...
logs = run_cmd(
    "find /data/sdt/buildlogs -mindepth 6 -maxdepth 6 -name 'addOnTests.zip'"
)
logs = logs[1].split("\n")
# process zip log files
for logFile in logs:
