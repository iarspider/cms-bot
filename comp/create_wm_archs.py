...
WM_ARCHS = {
    ...
    "rhel7_ppc64le": [
        "slc7_ppc64le_gcc820",
        {"python3": "3.8.2", "py2-future": "0.18.2"},
    ],
    "rhel7_aarch64": [
        "slc7_aarch64_gcc820",
        {"python3": "3.8.2", "py2-future": "0.18.2"},
...
def create_default_links(comp_ver, def_ver):
    if not exists(def_ver):
        cmd = "mkdir -p {0} && ln -s {1} {2}".format(
            dirname(def_ver), comp_ver, def_ver
        )


def find_cms_version(arch, pkg, ver):
    out = runcmd(
        "ls -d %s/%s/external/%s/%s*" % (CMS_CVMFS, arch, pkg, ver), debug=False
    )
    mver = ""
    for v in out.split("\n"):
        mver = v
...
def create_comp_package(arch, pkg, ver, cmspkg):
    ...
    if pkg == "python3":
        ...
        if not exists(def_future):
            ...
            runcmd(
                "mkdir -p {0} && ln -s {1} {2}".format(
                    dirname(def_future), comp_ver, def_future
                )
            )
    else:
