...
def update_worklog(workflow_dir, jobs):
    ...
    for job in jobs["commands"]:
        ...
        try:
            ...
            if m:
                ...
                m = re.match(
                    ".*\s*>\s*step([1-9][0-9]*)_[^\s]+\.log.*$", job["command"]
                )
                if m:
                    cmd_step = int(m.group(1))
                else:
                    cmd_step = int(
                        job["command"].split(" step", 1)[-1].strip().split(" ")[0]
                    )
    ...
    e, o = run_cmd(
        "grep ' exit: ' %s | sed 's|exit:.*$|exit: %s|'"
        % (workflow_logfile, exit_codes.strip())
    )
...
def update_known_error(worflow, workflow_dir):
    ...
    if worflow in known_errors:
        json.dump(
            known_errors[workflow], open("%s/known_error.json" % workflow_dir, "w")
        )
    return
