...
def createJob(workflow, cmssw_ver, arch):
    workflow_args = FixWFArgs(
        cmssw_ver, arch, workflow, GetMatrixOptions(cmssw_ver, arch)
    )
    cmd = format(
    ...
    try:
        workflow_dir = glob.glob(
            format("%(workflow)s/%(workflow)s_*", workflow=workflow)
        )[0]
        run_cmd(
            format(
...
while True:
    ...
    if (
        (not "hits" in stats)
        or (not "hits" in stats["hits"])
        or (not stats["hits"]["hits"])
    ):
