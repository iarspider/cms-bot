...
class CMSWeb(object):
    def __init__(self):
        ...
        self.conn = HTTPSConnection(
            self.URL_CMSWEB_BASE,
            cert_file="/tmp/x509up_u{0}".format(getuid()),
            timeout=30,
        )
        self.reply_cache = {}
        self.last_result = ""
    ...
    def search_dataset_status(self, dataset):
        status, jmsg = self.get_cmsweb_data(
            "{0}?{1}".format(
                ...
                urlencode(
                    {"detail": 1, "dataset_access_type": "*", "dataset": dataset}
                ),
            )
        )
        if not status:
    ...
    def search_files(self, dataset):
        status, jmsg = self.get_cmsweb_data(
            "{0}?{1}".format(
                self.URL_DBS_FILES, urlencode({"detail": 1, "dataset": dataset})
            )
        )
        if not status:
            return {}
    ...
    def search_blocks(self, dataset):
        status, jmsg = self.get_cmsweb_data(
            "{0}?{1}".format(
                self.URL_DBS_BLOCKS, urlencode({"dataset": dataset, "detail": 1})
            )
        )
        if not status:
            return {}
