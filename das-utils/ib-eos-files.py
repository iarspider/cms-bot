...
def get_lfns_from_das(lfn_per_query=1):
    ...
    for qfile in qfiles.split("\n"):
        ...
        lfn_count = 0
        err, out = run_cmd(
            "grep '/store/' %s" % lfn_file, debug=False, exit_on_error=False
        )
...
def copy_to_eos(lfn, log_file):
    cmd = "%s/copy-ib-lfn-to-eos.sh %s %s >%s 2>&1" % (
        CMS_BOT_DIR,
        lfn,
        opts.redirector,
        log_file,
    ...
    e, o = run_cmd(
        "grep ' echo ALL_OK' %s" % log_file, exit_on_error=False, debug=False
    )
    if "ALL_OK" in o:
        print("  Success: %s" % lfn)
    else:
...
def kill_xrootd(lfn):
    ...
    if pids:
        ...
        run_cmd(
            "%s rm %s%s.tmp" % (eos_cmd, eos_base, lfn),
            exit_on_error=False,
            debug=False,
        )
...
def eos_exists(eos_file):
    err, out = run_cmd(
        "%s stat -f %s" % (eos_cmd, eos_file), exit_on_error=False, debug=False
    )
    if err:
        return False
    return True
...
def eos_rename(name, new_name):
    ...
    err, out = run_cmd(
        "%s file rename %s %s" % (eos_cmd, name, new_name),
        exit_on_error=False,
        debug=False,
    )
    if err:
...
def eos_size(eos_file):
    if not eos_exists(eos_file):
        return -1
    err, out = run_cmd(
        "%s ls -l %s | awk '{print $5}'" % (eos_cmd, eos_file),
        debug=True,
        exit_on_error=False,
    )
...
def copy_lfns_to_eos(eos_lfns):
    ...
    for lfn in eos_lfns:
        ...
        if eos_exists(eos_file) or (
            eos_exists(eos_file + ".unused")
            and eos_rename(eos_file + ".unused", eos_file)
        ):
            already_done += 1
    ...
    for lfn in eos_lfns_to_copy:
        ...
        while True:
            ...
            if len(threads) < opts.jobs:
                ...
                print(
                    "Copy (%s/%s): %s" % (already_done + len(all_logs), total_lfns, lfn)
                )
