        ...
        if (len(fields) > 1) and (fields[0] == xf):
            try:
                res = (
                    res
                    + " ["
                    + ",".join([str(i) for i in item[xf][0][field_map[xf]]])
                    + "]"
                )
            except Exception as e:
                with open(efile, "w") as ofile:
...
if __name__ == "__main__":
    ...
    parser.add_option(
        "-q", "--query", dest="query", help="Only process this query", default=None
    )
    parser.add_option(
        "-d",
        "--debug",
    ...
    if opts.query:
        ...
        query = re.sub(
            "= ", "=", re.sub(" =", "=", re.sub("  +", " ", opts.query.strip()))
        )
    ...
    for query in query_sha:
        ...
        if exists(outfile):
            ...
            if (dtime <= opts.threshold) and (fcount > 0):
                ...
                if okcache:
                    print(
                        "  %s Found in cache with %s results (age: %s src)"
                        % (sha, fcount, dtime)
                    )
                    inCache += 1
                    continue
        ...
        while True:
            ...
            if tcount < jobs:
                ...
                try:
                    ...
                    print(
                        "ERROR threading das query cache: caught exception: " + str(e)
                    )
                    error += 1
                break
            else:
