...
if __name__ == "__main__":
    ...
    parser.add_option(
        ...
        "--days",
        dest="days",
        help="Files access in last n days",
        type=int,
        default=7,
    )
    parser.add_option(
        "-j", "--job", dest="job", help="Parallel jobs to run", type=int, default=4
    )
    parser.add_option(
        "-p",
        "--page",
    ...
    es_data = es_query(
        "ib-dataset-*", query, start_time, end_time, scroll=True, fields=["lfn"]
    )
