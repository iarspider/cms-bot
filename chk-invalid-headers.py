...
def main():
    ...
    usedby = {}
    readDeps(
        uses, join(environ["CMSSW_RELEASE_BASE"], "etc", "dependencies", "uses.out.gz")
    )
    readDeps(
        usedby,
        join(environ["CMSSW_RELEASE_BASE"], "etc", "dependencies", "usedby.out.gz"),
    )
    ...
    for inc in usedby:
        ...
        for src in usedby[inc].split(" "):
            ...
            if (
                (items[0] == sitems[0])
                and (items[1] == sitems[1])
                and (items[2] == sitems[2])
            ):
                continue
