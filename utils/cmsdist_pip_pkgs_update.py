                    ...
                    if exists(exfile):
                        with open(exfile) as xref:
                            for xline in xref.readlines():
                                m = re.match(
                                    "^%define\s+pip_name\s+([^\s]+)\s*$", xline.strip()
                                )
                                if m:
    ...
    for data in req_data:
        ...
        if True:
            ...
            if "python" in data["data"]:
                ...
                if False and not check_python_require(
                    py_ver, jdata["info"]["requires_python"]
                ):
                    releases = []
                    msg = []
                    for i in jdata["releases"]:
                        for d in jdata["releases"][i]:
                            ...
                            if not check_python_require(py_ver, d["requires_python"]):
                                ...
                                continue
                            uptime = (
                                datetime.strptime(d["upload_time"], "%Y-%m-%dT%H:%M:%S")
                                - epoch
                            ).total_seconds()
                            releases.append(
                                {
