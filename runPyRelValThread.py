...
def runStep1Only(basedir, workflow, args=""):
    args = FixWFArgs(
        os.environ["CMSSW_VERSION"], os.environ["SCRAM_ARCH"], workflow, args
    )
...
def runThreadMatrix(basedir, workflow, args="", logger=None, wf_err={}):
    ...
    for logFile in glob.glob(outfolder + "/step1_dasquery.log"):
        ...
        if not m:
            continue
        ret = doCmd(
            "cp " + logFile + " " + m.group(1) + "/step1_" + m.group(3) + ".log"
        )
    ret = doCmd(
        "mv "
...
def splitWorkflows(workflows, max_wf_pre_set):
    ...
    for i in range(1, wf_sets):
        ...
        while (new_avg > avg_t) and (new_index < long_wf):
            new_index += 1
            sub_set = (
                workflows[0 : long_wf - new_index] + workflows[-short_wf - new_index :]
            )
            new_avg = sum([x[1] for x in sub_set]) / len(sub_set)
        ...
        while len(workflows) > 0:
            ...
            if len(threads) < self.jobs:
                try:
                    ...
                    print(
                        "runPyRelVal> ERROR threading matrix step1 : caught exception: "
                        + str(e)
                    )
        for t in threads:
            t.join()
    ...
    def parseLog(self):
        ...
        for wf in logData:
            ...
            for step in sorted(logData[wf]["steps"]):
                ...
                json_cache = (
                    os.path.dirname(logFile) + "/logcache_" + str(step) + ".json"
                )
