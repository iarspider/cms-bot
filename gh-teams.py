...
REPO_TEAMS["cms-externals"]["Developers"] = deepcopy(
    REPO_TEAMS["cms-data"]["Developers"]
)
...
for org_name in CMS_ORGANIZATIONS:
    ...
    for team in teams:
        ...
        for repo in team_info["repositories"]:
            if repo == "*":
                repo_to_check += [r.name.encode("ascii", "ignore") for r in org_repos]
        ...
        for repo_name in set(repo_to_check):
            ...
            inv_repo = "!" + repo_name
            repo = [
                r for r in team_repos if r.name.encode("ascii", "ignore") == repo_name
            ]
            if (repo_name in team_info["repositories"]) or (
                "*" in team_info["repositories"]
                and (not inv_repo in team_info["repositories"])
            ):
                prem = "pull"
