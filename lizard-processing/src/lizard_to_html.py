...
def get_args():
    ...
    parser.add_argument(
        ...
        "--link_root",
        type=str,
        help="Project's repository at Github",
        required=True,
    )
...
def text_with_href(url_base, line):
    ...
    if bool(re.search(regex_line_to_url, line)):
        ...
        if bool(line_numbers_group):
            ...
            url = (
                url_base + line_split[1] + "#" + "L{0}-L{1}".format(lines[0], lines[1])
            )
...
def parse(f_out, line):
    ...
    if bool(re.search(regex_dashes, line)):
        ...
        table_row_values = re.split(
            regex_split_td, line.strip(), maxsplit=g_total_col_nr
        )
        row_dataset = []
