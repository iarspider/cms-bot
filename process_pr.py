...
def ignore_issue(repo_config, repo, issue):
    ...
    if issue.body:
        if re.search(
            CMSBOT_IGNORE_MSG,
            issue.body.encode("ascii", "ignore").split("\n", 1)[0].strip(),
            re.I,
        ):
            return True
    return False
...
def process_pr(repo_config, gh, repo, issue, dryRun, cmsbuild_user=None, force=False):
    ...
    if issue.pull_request:
        ...
        if cmssw_repo or not external_repo:
            ...
            packages = sorted(
                [
                    x
                    for x in set(
                        [cmssw_file2Package(repo_config, f) for f in chg_files]
                    )
                ]
            )
        ...
        print(
            "Latest commit by ",
            last_commit.committer.name.encode("ascii", "ignore"),
            " at ",
            last_commit_date,
        )
        ...
        if test_comment is not None:
            ...
            if (not bot_status) or (bot_status.target_url != turl):
                ...
                desc = "requested by %s at %s UTC." % (
                    test_comment.user.login.encode("ascii", "ignore"),
    ...
    if ack_comment:
        ...
        if (not state) or (state.target_url != ack_comment.html_url):
            desc = "Comment by %s at %s UTC processed." % (
                ack_comment.user.login.encode("ascii", "ignore"),
