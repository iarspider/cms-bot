...
BUILD_ERROR_MSG = (
    "The was an error for {architecture}. \n You can see the log here: \n {log_url}"
)
...
BASE_BUILD_LOG_URL = (
    ...
    + JENKINS_PREFIX
    + "-artifacts/auto-build-release/%s-%s/%d"
)
BASE_UPLOAD_LOG_URL = (
    "https://cmssdt.cern.ch/SDT/"
    + JENKINS_PREFIX
    + "-artifacts/auto-upload-release/%s-%s/%d"
)
BASE_CLEANUP_LOG_URL = (
    "https://cmssdt.cern.ch/SDT/"
    + JENKINS_PREFIX
    + "-artifacts/cleanup-auto-build/%s-%s/%d"
)
...
def get_test_log(logfile):
    ...
    try:
        ...
        try:
            logmsg = "\n\nTests results:\n" + getoutput(
                "grep 'ERROR\| tests passed' " + logfile
            )
        except:
...
if __name__ == "__main__":
    ...
    test_logfile = (
        "build/" + release_name + "-tests/matrixTests/runall-report-step123-.log"
    )
