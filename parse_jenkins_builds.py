...
jque_res = subprocess.run(
    que_cmd, shell=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE
)
        ...
        if os.path.exists(logFile) and not os.path.exists(flagFile):
            ...
            try:
                ...
                if queue_id in es_queue.keys():
                    ...
                    queue_payload["wait_time"] = (
                        int(jstime) - queue_payload["in_queue_since"]
                    )
    ...
    for dir in os.listdir(job_path):
        if dir.isdigit():
            ...
            if queue_id != None:
                ...
                es_queue[entry]["wait_time"] = (
                    int(jstime) - es_queue[entry]["in_queue_since"]
                )
