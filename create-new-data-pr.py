...
if __name__ == "__main__":
    ...
    parser = OptionParser(
        usage="%prog <cms-data-repo> <cms-dist repo> <pull-request-id>"
    )
    ...
    if create_new_tag:
        ...
        tag_ref = data_repo.create_git_ref(
            ...
            sha=data_repo.get_branch(data_pr_base_branch).commit.sha,
        )
    ...
    try:
        dist_repo.create_git_ref(
            ref="refs/heads/" + repo_tag_pr_branch, sha=sb.commit.sha
        )
